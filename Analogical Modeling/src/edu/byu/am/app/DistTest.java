...
public class DistTest {
		 ...
		 */
		public DistTest(String fileName) throws IOException {
			makeDataLoader();
			load(fileName);
		 ...
		 *             If an error occurs while reading fileName
		 */
		public DistTest(DataLoader dloader, String fileName) throws IOException {
			dl = dloader;
			load(fileName);
			card = data.get(0).size();
		}
		...
		private AnalogicalSet classify(Exemplar testItem) {
			...
			// 1. Place each data item in a distributed subcontext
			subList = new SubsubcontextList(testItem, data);
			System.out.println("sublist: " + subList);
			// 2. Place subcontexts into the supracontextual lattice
//			List<Lattice> ll = new LinkedList<Lattice>();
//			for(SubcontextList sl : subList){
//				ll.add(new Lattice(0, sl));//TODO: need to get cardinality!
//			}
...
//					Boolean.parseBoolean(Options.properties.getProperty("algorithm.pointers.linear")));
		...
		public static void main(String[] args) throws IOException {
			DistTest cl = new DistTest("ch3example.txt");
