...
public class AnalogicalModeling extends weka.classifiers.AbstractClassifier
	 ...
	 */
	private MissingDataCompare missingDataCompare = MissingDataCompare.VARIABLE;
	...
	public SelectedTag getMissingDataCompare() {
		return new SelectedTag(missingDataCompare.ordinal(), TAGS_MISSING);
	}
	...
	public void setMissingDataCompare(SelectedTag newMode) {
		if (newMode.getTags() == TAGS_MISSING) {
			missingDataCompare = MissingDataCompare.getElement(newMode);
		}
	}
	...
	public String[] getOptions() {
		...
		if (getRemoveTestExemplar())
			options.add("-R");
		options.add("-M");
		options.add(missingDataCompare.getOptionString());
