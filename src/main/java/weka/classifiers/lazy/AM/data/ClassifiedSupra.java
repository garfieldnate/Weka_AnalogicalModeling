...
public class ClassifiedSupra implements Supracontext {
    ...
    public void add(Subcontext sub) {
        if (supra.isEmpty()) outcome = sub.getOutcome();
        else if (!isHeterogeneous() && wouldBeHetero(sub)) outcome = AMUtils.HETEROGENEOUS;
        supra.add(sub);
    }
     ...
     * heterogeneous.
     *
     * @param sub subcontext to hypothetically add
