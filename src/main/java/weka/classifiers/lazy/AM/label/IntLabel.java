...
public class IntLabel extends Label {
    ...
    /**
     * @param l binary label represented by integer
    ...
    public IntLabel(Label other) {
        ...
        for (int i = 0; i < other.getCardinality(); i++)
            if (!other.matches(i)) labelBits |= (1 << i);
    ...
    /**
     * @return An integer whose 1 bits represent the mismatches and 0 bits represent the matches in this label.
     */
    ...
    public Label intersect(Label other) {
        if (!(other instanceof IntLabel)) throw new IllegalArgumentException(
    ...
    private class SubsetIterator implements Iterator<Label> {
        ...
        /**
         * Constructs and iterator over all subsets of this label
         */
        public SubsetIterator() {
            ...
            gaps = new int[card];
            List<Integer> gapsTemp = new ArrayList<>();
