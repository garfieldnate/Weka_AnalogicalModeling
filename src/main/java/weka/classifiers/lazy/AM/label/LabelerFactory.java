...
public class LabelerFactory {
	 ...
	 * @param mdc
	 *            The strategy for comparing missing data
	 * @param ignoreUnknowns
	 *            True if missing data should be ignored, false otherwise
	...
	public static Labeler createLabeler(Instance testInstance,
		...
		if (cardinality <= IntLabel.MAX_CARDINALITY)
			labeler = new IntLabeler(mdc, testInstance, ignoreUnknowns);
		else if (cardinality <= LongLabel.MAX_CARDINALITY)
			labeler = new LongLabeler(mdc, testInstance, ignoreUnknowns);
		else
			labeler = new BitSetLabeler(mdc, testInstance, ignoreUnknowns);
