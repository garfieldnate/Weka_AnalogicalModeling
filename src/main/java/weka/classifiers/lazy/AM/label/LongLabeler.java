...
public class LongLabeler extends Labeler {
	 ...
	 *             if the cardinality of the input instance is greater than
	 *             {@link LongLabel#MAX_CARDINALITY}.
	 */
	// TODO: since this throws an exception, perhaps a static factory method
	// would be better?
	public LongLabeler(MissingDataCompare mdc, Instance instance,
			boolean ignoreUnknowns) {
		super(mdc, instance, ignoreUnknowns);
		if (getCardinality() > LongLabel.MAX_CARDINALITY)
	...
	public Label partition(Label label, int partitionIndex) {
		...
		if (!(label instanceof LongLabel))
			throw new IllegalArgumentException("This labeler can only handle "
					+ LongLabel.class.getCanonicalName()
	...
	private class BitMask {
		...
		public Label mask(LongLabel label) {
			LongLabel longLabel = new LongLabel(
					(maskBits & label.labelBits()) >> startIndex, cardinality);
